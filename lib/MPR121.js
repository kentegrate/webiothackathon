...
module.exports = (i2cPort) => {
  ...
  getPinState: (pin) => {
    return mpr121.isTouched(pin);
  },

  addEventHandler: function(label, func) {
    mpr121.eventHandler[label].push(func);
  },

  clearEventHandler: function(label) {
    mpr121.eventHandler[label] = [];
  }
};
...
MPR121.prototype = {
  ...
  updateState: function(touched) {
    ...
    self.state.forEach((previous, i) => {
      ...
      self.eventHandler.stateChange.forEach((func) => {
        func(i, current);
      });
