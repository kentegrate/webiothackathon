...
module.exports = (i2cPort) => {
  ...
  mpr121.init().then(() => {
    console.log("init mpr121");
  });
  setInterval(() => {
    for(let i = 0; i < 12; i++) {
      if(mpr121.isTouched(i)) {
        console.log(i.toString() + " pin is Touched");
      }
    }
  });
};
...
MPR121.prototype = {
  ...
  configure: function() {
    ...
    return self.setThresholds(12, 6)
      .then(() => self.write8(MPR121_MHDR, 0x01))
      .then(() => self.write8(MPR121_NHDR, 0x01))
      .then(() => self.write8(MPR121_NCLR, 0x0E))
      .then(() => self.write8(MPR121_FDLR, 0x00))
      .then(() => self.write8(MPR121_MHDF, 0x01))
      .then(() => self.write8(MPR121_NHDF, 0x05))
      .then(() => self.write8(MPR121_NCLF, 0x01))
      .then(() => self.write8(MPR121_FDLF, 0x00))
      .then(() => self.write8(MPR121_NHDT, 0x00))
      .then(() => self.write8(MPR121_NCLT, 0x00))
      .then(() => self.write8(MPR121_FDLT, 0x00))
      .then(() => self.write8(MPR121_DEBOUNCE, 0))
      .then(() => self.write8(MPR121_CONFIG1, 0x10)) // default, 16uA charge current
      .then(() => self.write8(MPR121_CONFIG2, 0x20)) // 0.5uS encoding, 1ms period
      .then(() => {
        this.ready = true;
        console.log('MPR121 ready');
      });
  },
