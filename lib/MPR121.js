...
module.exports = (i2cPort) => {
  var mpr121 = new MPR121(i2cPort, 0x5A, 1000);
  return mpr121.init().then(() => {
    log("init MPR121");
    return mpr121;
  });
};
...
MPR121.prototype = {
  ...
  updateState: function(touched) {
    ...
    self.state.forEach((previous, i) => {
      ...
      self.eventListener.stateChange.forEach((func) => {
        func(i, current);
      });
  ...
  getPinState: function(pin) {
    var self = this;
    return self.isTouched(pin);
  },

  addEventListener: function(label, func) {
    var self = this;
    self.eventListener[label].push(func);
  },

  clearEventListener: function(label) {
    var self = this;
    self.eventListener[label] = [];
  }
};
